...
read_row = utility.countrows(file=file, sheetname='Sheet1')-1
...
for row in range(1,read_row+1):
    ...
    def chandan(country_code,zip_code):
        response = requests.get(f"http://api.zippopotam.us/{country_code}/{zip_code}")
        response_body = response.json()

        if response.status_code ==200:
            assert response.status_code ==200 , print("ooooo")

            utility.writeData(file=file,sheetname='Sheet1',row=row,column=6,value='pass')
            print("pass")
        else:
